...
const TIME = Platform.OS === 'ios'?10:20; //每次滚动的时间间隔
const DISTANCE = Platform.OS === 'ios'?2:5; //每次滚动的距离
...
const MemoDragSortableView = (props, ref) => {
	...
	const startScroll = (distance) => {
		if(!scrollTimeRef.current){
			...
			scrollTimeRef.current = setInterval(() => {
				//判断是否应该滚动，还是停止滚动
				if(Math.floor(scrollYRef?.current) <= Math.ceil(yRef.current) && distance < 0){
					stopScroll();
					return;
				}
				
				if(Math.ceil(scrollYRef?.current) >= Math.floor((yRef.current + heightRef.current) - scrollViewHeightRef?.current) && distance > 0){
					stopScroll();
					return;
				}
				scrollYRef.current += distance
	...
	const isOutScrollView = () => {
		return scrollViewRef?.current && scrollViewHeightRef?.current && scrollYRef?.current;
	}
	...
	const dragEnd = (index, e) => {
		...
		//计算索引变化，将变化后数据吐出去
		if(dragData.originIndex !== dragData.changeIndex){
			let arr = [];
